...
def test_dataset(args,deblur_model,pred_model,test_loader,save_path=None):
    ssim_sum = 0
    mse_sum = 0
    psnr_sum = 0
    ...
    for j, (gt, im) in enumerate(test_loader):
        ...
        with torch.no_grad():
            pred_core = pred_model(im)
            if args.model == 'UNet':
                output = deblur_model(im, pred_core)
            if args.model == 'Transformer':
                restored = deblur_model(im, pred_core) 
                output = torch.clamp(restored,0,1) 
...
def test_state(args,state_dict1,state_dict2):
    ...
    deblur_model.cuda()
    deblur_model.eval()
    deblur_model.load_state_dict(state_dict1)
    del(state_dict1)
    pred_model = nn.DataParallel(pred_model)
    pred_model.cuda()
    pred_model.eval()
    pred_model.load_state_dict(state_dict2)
    del(state_dict2)
    save_path = None
    ...
    if args.save_result_path == True:
        ...
        if args.test_path == './RealDoF/':
            save_path = './result_realdof' + args.model
    num, ssim_av,psnr_av,mse_av = test_dataset(args,deblur_model,pred_model,test_loader_dpdd,save_path)

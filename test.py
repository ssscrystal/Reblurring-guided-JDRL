...
parser.add_argument('--checkpoint_path',default="weights/sdd_jdrl_transformer.pth",help="path to checkpoint")
...
parser.add_argument('--model',default='Transformer',help="Transformer or UNet")
...
for i in range(len(ckpt_list)):
    ...
    if args.model == 'UNet':
        state1 = ckpt['G_state_dict']
        state2 = ckpt['Pred_state_dict']
    if args.model == 'Transformer':
        state1 = ckpt['G_state_dict']
        state2 = ckpt['Pred_state_dict']
    ssim_av,psnr_av,mse_av = test_state(args,state1,state2)
    del(ckpt)
